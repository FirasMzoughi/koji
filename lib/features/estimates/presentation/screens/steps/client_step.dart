...
class _ClientInfoStepState extends ConsumerState<ClientInfoStep> {
  ...
  void initState() {
    ...
    _nameController = TextEditingController(text: draft.client?.name ?? '');
    _addressController = TextEditingController(text: draft.client?.address ?? '');
    _phoneController = TextEditingController(text: draft.client?.phone ?? '');
  }
  ...
  void _update() {
    ref.read(draftEstimateProvider.notifier).updateClient(
